...
> ⚠️ **版本敏感性**：本文对 Claude Code 内部细节的论断（OSC 码与 `status` 取值、JSONL 路径/字段、agents/swarm 编排）核对自一个**可能偏旧**的源码快照。设计有意**架在跨版本稳定的不变量上**（PTY 字节流、行内 `sessionId`/`cwd` 戳、tmux 自省、文件系统观测），把易变细节降级为「实现时按**目标 claude 版本**现场复核」。实现阶段应对照实际安装版本验证这些常量，变了只需调参数、不动架构。
...
## 3.5 多会话 / 一终端管多 session（agents UI）

claude 的 **agents UI** 支持一个终端编排 / 管理多个并发会话。

> ⚠️ **版本敏感**：参考的 `/root/claude-src` 快照偏旧，该模式的**具体实现可能已变**（是否 tmux 分屏、是否独立进程、有无 `teams/<name>/config.json` 索引及其字段、文件落点）。故本节**以跨版本稳定的不变量为骨架**，把任何具体 schema/路径降级为「实现时按目标 claude 版本现场复核」的可选提示，**不硬依赖**旧源码细节。

**依赖的稳定不变量**（多个大版本未变、可放心建模）：
1. 每个会话各有一份 JSONL，落在 `~/.claude/projects/<sanitized-cwd>/` 下（顶层 `<sessionId>.jsonl` 或会话子目录内）。
2. **每行都盖 `sessionId` + `cwd`**（多会话时通常还有 `teamName`/`agentName`/`agentColor`，§3.2.1）—— 这是会话身份的**唯一真值**，与具体编排实现无关。
3. 终端最终画面就是 pane 的 ANSI 流。

**据此分通道（实现优先级：通道一 v1 已白送，通道二/三 fan-out 为 v2）**：

- **通道一（屏幕）：模式无关、零改动、最稳**。无论 agents UI 是分屏、切屏选择器、还是单屏多 tab，tmux client 渲染的就是它的最终画面 —— 远程 Dashboard + 本地 attach **透明可见可驱动，多会话编排仅靠通道一即可完整远程操作**。这条**不依赖任何版本细节**，是多会话支持的底线保证。
- **通道二：靠「文件系统观测 + 行内戳」发现会话集合，不靠固定 schema**。在 cwd 项目目录树里找**所有并发增长的 `*.jsonl`**（排除 `*.meta.json`/timeline/bridge-pointer），各自**独立换行游标并发 tail**；每条 transcript 用其**行内** `sessionId`/`teamName`/`agentName`/`color` 标注 → Dashboard 分流或树形展示。会话集合随文件增删动态进出。
  - *可选加速*：若目标版本存在编排索引（如旧源码的 `~/.claude/teams/<name>/config.json` 的 `members[].sessionId`），可读它直接拿到会话清单与 `tmuxPaneId` 映射；**拿不到/格式变了就退回纯文件观测**，不阻断。
- **通道三 / 焦点：优先用 tmux 自省（与 claude 版本无关）**。`tmux -L claude-ctl list-panes -F '#{pane_id} #{pane_active} #{pane_pid} #{pane_title}'` 拿焦点 pane 与每 pane 标题，避免在多 pane 合流字节里猜哪个发的 OSC；发 `{"type":"session_focus", paneId, sessionId?}`。pane→sessionId 的映射若无索引可best-effort（按 pane `cwd`/`pid` 关联），拿不到就只报焦点 paneId、不强行映射。
- **多实例消歧**沿用 §3.2.1（本 pane `tab_status` 活动窗口 / 行内 `sessionId` 一致性）。

> 这样设计的好处：**通道一对"一终端多 session"天然可用且与版本解耦**；通道二/三的结构化 fan-out 即便目标版本改了编排实现，只要不变量 1–3 成立就照常工作，最坏退化为"按文件观测多个 sessionId"。会话身份始终以**每行 `sessionId`** 为准。
...
5. **M5（v2 / 可选）**：**多会话 fan-out**（并发 tail + tmux 焦点映射，§3.5）+ 子代理嵌套 tail（§3.4）+ vt100 重连快照 + notify + base64 模式 + 审计日志 + systemd watchdog（§12）。
6. **跨阶段校验**：对照**目标 claude 版本**复核版本敏感常量（OSC/status、JSONL 路径与字段、swarm 是否复用继承的 `$TMUX`）—— 见顶部「版本敏感性」。
